      ...
      if (options.detect_anomalies && data.length > 0) {
        ...
        data.forEach((point) => {
          ...
          if (zScore >= 2.3) {
            ...
            const description = Number.isFinite(zScore)
              ? `Value ${point.value.toFixed(2)} deviates ${zScore.toFixed(2)} standard deviations from mean`
              : `Value ${point.value.toFixed(2)} deviates significantly from mean`;
